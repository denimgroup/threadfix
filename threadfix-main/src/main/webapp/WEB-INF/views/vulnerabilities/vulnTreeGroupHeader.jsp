<div class="accordion-header" style="height:40px;padding: 8px 1px 4px 10px;" ng-show="element.totalVulns > 10 || treeApplication">
    ...
    <span ng-if="element.totalVulns < 11">
        <ul style="width:190px; float:left" class="nav nav-pills">
            <li id="unclickableShow10{{ category.name }}{{ element.genericVulnerability.displayId }}" ng-class="{ active: element.numberToShow === 10 }"> <a ng-disabled="true">10</a></li>
            <li id="unclickableShow25{{ category.name }}{{ element.genericVulnerability.displayId }}" ng-class="{ active: element.numberToShow === 25 }"> <a ng-disabled="true">25</a></li>
            <li id="unclickableShow50{{ category.name }}{{ element.genericVulnerability.displayId }}" ng-class="{ active: element.numberToShow === 50 }"> <a ng-disabled="true">50</a></li>
            <li id="unclickableShow100{{ category.name }}{{ element.genericVulnerability.displayId }}" ng-class="{ active: element.numberToShow === 100 }"><a ng-disabled="true">100</a></li>
        </ul>
    </span>
    ...
    <span ng-show="treeApplication" class="vuln-tree-checkbox">
        <input id="checkCategory{{ category.name }}{{ element.genericVulnerability.displayId }}" type="checkbox" ng-model="element.checked" ng-change="applyElementChecked(element)"/>
    </span>
</div>