...
<body id="formErrors">
	...
	<form:form style="margin-bottom:0px;" id="myAppForm${ org.id }" modelAttribute="application" method="post" autocomplete="off" action="${fn:escapeXml(saveUrl)}">
		...
		<div class="modal-footer">
			...
			<a id="submitAppModal" class="btn btn-primary" onclick="javascript:submitAjaxModal('<c:out value="${saveUrl }"/>','#myAppForm${ org.id }', '#formDiv${ org.id }', '#teamTable', '#myTeamModal${ org.id }', '#collapse${ org.id }');return false;">Add Application</a>
		</div>
	</form:form>
</body>