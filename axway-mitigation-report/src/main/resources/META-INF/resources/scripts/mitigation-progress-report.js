...
myAppModule.directive('d3Mitigation', ['d3',
    function(d3) {
        return {
            restrict: 'EA',
            scope: {
...
myAppModule.factory('mitigationUtils', function() {
    ...
    mitigationUtils.refreshVulns = function($scope){
        ...
        $scope.mitigationVulnsData.results = [];
        $scope.mitigationVulnsData.scanners = $scope.config;
        $scope.mitigationVulnsData.totalNumVuln = totalNumVuln;
        ...
        for(i = 0; i < scannerNames.length; i++){
            ...
            $scope.mitigationVulnsData.results.push(angular.copy($scope.scan));
        }

        angular.forEach($scope.mitigationVulnsData.scanners, function(value){
            if(value.criticalVulns == true || value.highVulns == true || value.mediumVulns == true ||
                value.lowVulns == true || value.infoVulns == true || value.auditable == true) {
        ...
        $scope.mitigationVulnsData.showEditModal = function(){
            $scope.showEditModal();
        };

        $scope.mitigationVulnsData.viewScan = function(id) {
            $scope.viewScan(id);
        };

    };

    return mitigationUtils;
});
